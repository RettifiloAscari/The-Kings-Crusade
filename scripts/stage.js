...
// when the scripts round-tripped to a second surface. Hardcoding it meant the
// path could only be changed in all generators at once, and a generator missed
// in that pass produces a partial corpus silently. It is a parameter now.
//
// Resolution order:
//   1. $KC_STAGE           -- what tools/build.sh sets
...
const STAGE = process.env.KC_STAGE || path.join(__dirname, '..', '.stage');
